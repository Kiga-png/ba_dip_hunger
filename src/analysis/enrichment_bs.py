...
from utils import add_motif_count, add_norm_log_ngs_read_count, add_dfname, add_selector, get_selctors, add_dvg_sequence, add_ngs_percentile_rank, add_ngs_percentile_rank_list
...
def create_motif_histogram_plot_background(dfname: str, n_rows: int, df: pd.DataFrame, motif_length: int, name_mod: str = ""):
    ...
    ax.set_title(f"dataset: {name_mod}{dfname}    motif length: {motif_length}     number of DVGs: {n_rows}")
    ax.set_xlabel("motif")
    ax.set_ylabel("relative frequency (%)")
    ax.tick_params(axis='x', labelrotation=90)
    ax.set_ylim(0, 10)
    ...
    save_path = os.path.join(RESULTSPATH, f"enrichment/{dfname}")
    os.makedirs(save_path, exist_ok=True)
    fname = f"motif_bg_{motif_length}_his_{name_mod}{dfname}.png"
    plt.savefig(os.path.join(save_path, fname), dpi=300)
    plt.close()
...
def create_motif_histogram_plot(dfname: str, n_rows: int, dfs: list[pd.DataFrame], motif_length: int, name_mod: str = ""):
    ...
    save_path = os.path.join(RESULTSPATH, f"enrichment/{dfname}")
    os.makedirs(save_path, exist_ok=True)
    fname = f"motif_{motif_length}_his_{name_mod}{dfname}.png"
    plt.savefig(os.path.join(save_path, fname), dpi=300)
    plt.close()
