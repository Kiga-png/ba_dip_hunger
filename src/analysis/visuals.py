...
sys.path.insert(0, "..")
from utils import get_dataset_names, load_all, get_sequence
from utils import manage_specifiers, load_all_preprocessed
from utils import rename_feature, add_feature_percentile_rank, add_log_feature, add_norm_feature, manage_intersects
...
def run_site_motif_heatmap_analysis(dfs: list, top_n: int = 10, data: str = '', strain: str = '', segment: str = '', intersects: str = ''):
    '''
    ...
    plot_names = ['before_deletion', 'deletion_start', 'deletion_end', 'after_deletion']
    ...
    for motif_length in range(1, max_motif_length + 1):
        df_copy = df.copy()
        motif_df, _= add_site_motifs(df_copy, motif_length)
    ...
    create_metric_spine_plot('mean_frequency_difference', 'motif_length', '-', plot_names, all_mean_freq_diffs, 'mean frequency difference (%)', dvg_count, data, strain, segment, intersects)
    create_metric_spine_plot('top_frequency_difference', 'motif_length', '-', plot_names, all_top_freq_diffs, 'top frequency difference (%)', dvg_count, data, strain, segment, intersects)

def run_reg_site_motif_heatmap_analysis(dfs: list, top_n: int = 10, data: str = '', strain: str = '', segment: str = '', intersects: str = ''):
    '''
    ...
    df = add_feature_percentile_rank(df, 'NGS_read_count', 'NGS_percentile_rank')
    reg_plot_names = ['reg_before_deletion', 'reg_deletion_start', 'reg_deletion_end', 'reg_after_deletion']
    ...
    create_metric_spine_plot('mean_reg_coefficients', 'motif_length', '-', reg_plot_names, all_mean_coefficients, 'mean regression coefficients (-)', dvg_count, data, strain, segment, intersects)
    create_metric_spine_plot('top_reg_coefficients', 'motif_length', '-', reg_plot_names, all_top_coefficients, 'top regression coefficients (-)', dvg_count, data, strain, segment, intersects)
...
def make_repeat_heatmap_analysis(df: pd.DataFrame, heatmap_name: str, dvg_count: int, cap_length: int, data: str, strain: str, segment: str, intersects: str):
    ...
    repeat_count_heatmap_df, percentile_rank_count_df = compute_feature_count_heatmap_df(df, 'direct_repeat_length')
    repeat_count_df = compute_feature_count_df(repeat_count_heatmap_df, 'direct_repeat_length')
...
def make_mfe_heatmap_analysis(df: pd.DataFrame, heatmap_name: str, dvg_count: int, data: str, strain: str, segment: str, intersects: str):
    ...
    mfe_count_heatmap_df, percentile_rank_count_df = compute_feature_count_heatmap_df(df, 'MFE_percentile_rank')
    mfe_count_df = compute_feature_count_df(mfe_count_heatmap_df, 'MFE_rank')
